...
------------
  ...
  We developed a python script to obtain GO and EC annotation (`go_ec_uniprot_annotator`) from a blast search against UniProt Database. This is achieved by parsing the UniProtIDs from a blast tabular file in the UniProt API to recover GO terms, EC numbers and protein names. `go_ec_uniprot_annotator` automates the [Retrieve/ID mapping](https://www.uniprot.org/id-mapping) from UniProt to obtain the annotation data. `go_ec_uniprot_annotator` priorizes SwissProt information, and thus it is advised to provide a separate blast search against TrEMBL and SwissProt. The annotation files generated by `go_ec_uniprot_annotator` are ready to use with `GODEG` and `ECDEG`.
