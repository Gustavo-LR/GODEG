...
------------
  ...
  Similarly to `GODEG`, we also implemented a pipeline to work with EC numbers: `ECDEG`. `ECDEG` is a python script pipeline to annotate EC numbers using UniProt database information (using `go_ec_uniprot_annotator`), apply cutoffs for DEG identification, classify DEGs in the complete EC hierarchy and perform an enrichment analysis to identify overrepresented EC numbers containing DEGs. The EC hierarchy is deductible from the EC number, and the EC names are available in the [expasy enzyme database](https://enzyme.expasy.org/), which are fetched automatically by `ECDEG`.
