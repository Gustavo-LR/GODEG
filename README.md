...
------------
  ...
  `GODEG` uses two main file inputs, a DEG table containing fold change and adjusted p values (typically resulted from software as DESEQ2 and EdgeR) and the complete genes GO annotation file. The DEG table is filtered to obtain DEGs based on adjusted p value and foldchange cutoffs, and these DEGs are assigned to GO categories (specific and parental categories). Typically, GO annotations contain the most specific GO terms. To obtain the ancestor GO categories we used the R package GO.db from bioconductor. After assigning the DEGs to GO terms, an enrichment analysis is performed to identify overrepresented GO terms containing DEGs.
